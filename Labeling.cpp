...
int main() {
	Mat img_color = imread("label.jpg", IMREAD_COLOR);
	Mat img_gray;
	...
	for (int i = 0; i < nlabels; i++) {
		...
		int center_y = centroids.at<double>(i, 0);
		...
		if (area > 50) {
			...
			putText(img_color, to_string(i), Point(left + 20, top + 20), FONT_HERSHEY_SIMPLEX, 1, Scalar(255, 0, 0), 3);
		}
	}
